...
const getSortedDocuments = async (query, sortField = 'timestamp', order = 'desc') => {
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const historySnap = await db.collection('applicationHistory')
...
router.patch('/:id/status', async (req, res) => {
  try {
    ...
    const validStatuses = ['pending', 'reviewed', 'shortlisted', 'interview', 'accepted', 'rejected', 'withdrawn'];
    ...
    if (status === 'accepted' || status === 'rejected') {
      ...
      if (jobDoc.exists) {
        ...
        if (status === 'accepted') {
          updateJobData.acceptedApplicants = admin.firestore.FieldValue.increment(1);
        } else if (status === 'rejected') {
          updateJobData.rejectedApplicants = admin.firestore.FieldValue.increment(1);
        }
...
router.patch('/:id/notes', async (req, res) => {
  try {
    ...
    // Add note to application's notes array
...
router.get('/stats/overview', async (req, res) => {
  try {
    ...
    if (applications.length > 0) {
      stats.conversionRate = ((stats.applicationsByStatus.accepted / applications.length) * 100).toFixed(2);
    }
    ...
    // Convert to array
    stats.dailyApplications = Object.entries(dailyData).map(([date, count]) => ({
      date,
      count
    }));
...
router.get('/export', async (req, res) => {
  try {
    ...
    filteredApplications.sort((a, b) => {
      const timeA = a.appliedAt?.toDate ? a.appliedAt.toDate() : new Date(a.appliedAt);
      const timeB = b.appliedAt?.toDate ? b.appliedAt.toDate() : new Date(b.appliedAt);
      return timeB - timeA;
    });
      ...
      filteredApplications.map(async (app) => {
        try {
          ...
          const exportData = {
            ...
            appliedAt: app.appliedAt?.toDate ? app.appliedAt.toDate().toISOString() : app.appliedAt,
            status: app.status,
          ...
          console.error(`Error enriching application ${app.id} for export:`, error);
          return {
            applicationId: app.id,
    ...
    if (format === 'csv') {
      ...
      res.json({
        success: true,
        message: 'Excel export requires additional setup. Use CSV or JSON format.',
